...
class TodoApp {
    ...
    cacheDOM() {
        ...
        // About modal
        this.aboutModal = document.getElementById('aboutModalOverlay');
        this.aboutLink = document.getElementById('aboutLink');
        this.closeAboutModalBtn = document.getElementById('closeAboutModalBtn');
        ...
        // Export modal
        this.exportModal = document.getElementById('exportModalOverlay');
        this.exportFooterLink = document.getElementById('exportFooterLink');
    ...
    bindEvents() {
        ...
        this.closeAboutModalBtn.addEventListener('click', () => this.closeAboutModal());
        ...
        // Export modal events
        this.exportFooterLink.addEventListener('click', (e) => {
            e.preventDefault();
    ...
    clearAllData() {
        if (confirm('Are you sure you want to delete ALL tasks? This action cannot be undone!')) {
            ...
            this.saveToStorage();
            this.render();
            this.showNotification('success', 'All data cleared!', 'All tasks have been removed.');
        }
    }
    ...
    getFilteredTasks() {
        ...
        switch (this.currentSort) {
            ...
            case 'priority':
                ...
                filteredTasks.sort((a, b) => priorityOrder[b.priority] - priorityOrder[a.priority] || new Date(b.createdAt) - new Date(a.createdAt));
                break;
        }
    ...
    updateProgress() {
        ...
        this.productivityScoreEl.textContent = `${Math.round(dailyProgress)}%`;
    ...
    saveSettings() {
        this.dailyGoal = parseInt(this.dailyGoalInput.value) || 5;
    ...
    updateExportPreview() {
        ...
        let tasksToExport = this.tasks.filter(task => {
            if (includeCompleted && includePending) return true;
            if (includeCompleted && task.completed) return true;
            if (includePending && !task.completed) return true;
            return false;
        });
        
        // Format data in Text format
        let exportData = "📋 TASKMASTER PRO - TASKS LIST 📋\n";
        exportData += "===================================\n\n";
        
        if (tasksToExport.length === 0) {
            exportData += "No tasks to export.\n";
        } else {
            exportData += `Total Tasks: ${tasksToExport.length}\n`;
            exportData += `Generated: ${new Date().toLocaleString()}\n\n`;
            exportData += "TASKS:\n";
            exportData += "======\n\n";
            
            tasksToExport.forEach((task, index) => {
                exportData += `${index + 1}. ${task.completed ? '✅ [COMPLETED]' : '⭕ [PENDING]'} ${task.text}\n`;
                
                if (includeMetadata) {
                    exportData += `   Priority: ${task.priority.toUpperCase()}\n`;
                    exportData += `   Created: ${new Date(task.createdAt).toLocaleDateString()}\n`;
                    
                    if (task.dueDate) {
                        exportData += `   Due Date: ${new Date(task.dueDate).toLocaleDateString()}\n`;
                    }
                    
                    if (task.updatedAt) {
                        exportData += `   Last Updated: ${new Date(task.updatedAt).toLocaleDateString()}\n`;
                    }
                }
            ...
            // Add summary
            const completedCount = tasksToExport.filter(t => t.completed).length;
            const pendingCount = tasksToExport.filter(t => !t.completed).length;
            
            exportData += "SUMMARY:\n";
            exportData += "========\n";
            exportData += `Total Tasks: ${tasksToExport.length}\n`;
            exportData += `Completed: ${completedCount}\n`;
            exportData += `Pending: ${pendingCount}\n`;
            exportData += `Completion Rate: ${tasksToExport.length > 0 ? Math.round((completedCount / tasksToExport.length) * 100) : 0}%\n\n`;
        }
        
        exportData += "---\n";
        exportData += "Exported from TaskMaster Pro\n";
        exportData += "© 2025 TaskMaster Pro\n";
        
        this.exportPreview.value = exportData;
    }
    
    copyExportToClipboard() {
        this.exportPreview.select();
        document.execCommand('copy');
        
        // Show visual feedback
        const originalText = this.copyExportBtn.innerHTML;
        this.copyExportBtn.innerHTML = '<i class="fas fa-check"></i> Copied!';
        ...
        this.showNotification('success', 'Copied!', 'Tasks copied to clipboard in Text format.');
    }
    ...
    downloadExport() {
        ...
        this.showNotification('success', 'Export successful!', 'Tasks exported as Text file.');
    }
